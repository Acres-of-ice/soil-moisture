...
static esp_err_t send_chunk_with_retry(httpd_req_t *req, const char *chunk,
  ...
  while (retries < MAX_SEND_RETRIES) {
    ...
    ESP_LOGW(TAG, "Failed to send chunk, retry %d/%d", retries + 1,
             MAX_SEND_RETRIES);
...
static esp_err_t http_server_generic_handler(httpd_req_t *req) {
  ...
  char *chunk =
      heap_caps_malloc(OPTIMAL_CHUNK_SIZE, MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
  if (!chunk) {
  ...
  if (ret == ESP_OK) {
    httpd_resp_send_chunk(req, NULL, 0);
    ESP_LOGI(TAG, "File sent successfully: %s (%d bytes)", filepath,
             bytes_sent);
  }
...
esp_err_t http_server_OTA_update_handler(httpd_req_t *req) {
  ...
  // message about the status if (flash_successful) {
  // http_server_monitor_send_message(HTTP_MSG_OTA_UPDATE_SUCCESSFUL); } else {
...
static httpd_handle_t http_server_configure(void) {
  ...
  if (httpd_start(&http_server_handle, &config) == ESP_OK) {
    ...
    httpd_uri_t generic_handler = {.uri = "/*",
                                   .method = HTTP_GET,
...
void http_server_start(void) {
  ...
  if (free_heap < MIN_HEAP_SIZE_HTTP) {
    ESP_LOGW(TAG, "Insufficient heap for HTTP server (%lu < %d)", free_heap,
             MIN_HEAP_SIZE_HTTP);
...
void http_server_stop(void) {
  if (http_server_handle) {
    vTaskDelay(
        pdMS_TO_TICKS(500)); // Give time for pending requests to complete
...
/**
 * Checks the g_fw_update_status and creates the fw_update_reset timer if
 * g_fw_update_status is true.
 */
void http_server_fw_update_reset_timer(void) {
  if (g_fw_update_status == OTA_UPDATE_SUCCESSFUL) {
    ESP_LOGI(TAG, "http_server_fw_update_reset_timer: FW updated successful "

...
static void espnow_recv_cb(const esp_now_recv_info_t *recv_info,
  ...
  if (sscanf(recv_str,
             "ID[%*02X:%*02X:%*02X:%*02X:%*02X:%*02X]S[%d]B[%d]T[%d]D[%19[^]]",
             &recv_data.soil_moisture, &recv_data.battery_level,
             &recv_data.temperature, recv_data.timestamp) == 4) {
...
//     ESP_ERROR_CHECK(esp_wifi_set_mode(WIFI_MODE_APSTA)); // APSTA mode for
//     both AP and ESP-NOW
...
//     ESP_ERROR_CHECK(esp_wifi_set_max_tx_power(84)); // Maximum transmission
//     power ESP_ERROR_CHECK(
//         esp_wifi_set_channel(CONFIG_ESPNOW_CHANNEL, WIFI_SECOND_CHAN_NONE));

//     ESP_LOGI(TAG, "WiFi AP started: SSID=%s, Password=12345678,
//     IP=192.168.4.1",
//              ssid);
// }
...
//   // if (status == ESP_NOW_SEND_SUCCESS) {
//   //   ESP_LOGD(TAG, "ESP-NOW message sent successfully to %s (0x%02X)",
//   pcb_name,
//   //            device_addr);
...
//       ESP_LOGI(TAG, "Relay %d activated (%s)", relay, relay == 1 ? "POSITIVE"
//       : "NEGATIVE");
...
//         message->retries); vTaskDelay(pdMS_TO_TICKS(20));
//       }
...
void processPumpMessage(comm_t *message) {
  ...
  for (int retry = 0; retry < MAX_RETRIES / 2; retry++) {
    ESPNOW_queueMessage(CONDUCTOR_ADDRESS, ack_cmd, message->address,
                        message->retries);
    vTaskDelay(pdMS_TO_TICKS(20));
  }
...
// void custom_recv_cb(const uint8_t *mac_addr, const uint8_t *data, int
// data_len, int rssi) {
...
//   //     char *count_start = strstr(msg, " Count:");
//   //     if (pcb_start && count_start && count_start > pcb_start) {
//   //         int name_len = count_start - (pcb_start + 4); // 4 =
//   strlen("PCB:")
//   //         if (name_len > 0 && name_len < sizeof(recv_data.pcb_name)) {
//   //             strncpy(recv_data.pcb_name, pcb_start + 4, name_len);
//   //             recv_data.pcb_name[name_len] = '\0';
...
//   //     ESP_LOGI(TAG, "Parsed (memcpy method): M=%d%%, B=%d%%, T=%d°C |
//   Time: %s",
//   //              moisture, battery, temp, timestamp);
//     // Check for command messages
//   if (data_len > 0) {
  ...
  for (int i = 0; i < peer_count; i++) {
    ...
    if (espnow_get_peer_mac(i, mac_addr) == ESP_OK) {
      ...
      if (pcb_name != NULL && strncmp(pcb_name, "Unknown-", 8) != 0) {
        ...
        //   } else if (strcasestr(pcb_name, "drain") != NULL) {
        //     ESP_LOGI(TAG,
        //              "  Partial match 'drain' -> DRAIN_NOTE_ADDRESS
        //              (0x%02X)", DRAIN_NOTE_ADDRESS);
        //     register_device_mac_mapping(DRAIN_NOTE_ADDRESS, mac_addr);
        //   } else if (strcasestr(pcb_name, "source") != NULL) {
        //     ESP_LOGI(TAG,
        //              "  Partial match 'source' -> SOURCE_NOTE_ADDRESS
        //              (0x%02X)", SOURCE_NOTE_ADDRESS);
        //     register_device_mac_mapping(SOURCE_NOTE_ADDRESS, mac_addr);
        //   } else if (strcasestr(pcb_name, "air") != NULL) {
        //     ESP_LOGI(TAG, "  Partial match 'air' -> AIR_NOTE_ADDRESS
        //     (0x%02X)",
        //              AIR_NOTE_ADDRESS);
...
esp_err_t espnow_init2(void) {
  ...
  // for (int i = 0; i < 5; i++) {
  //   ESP_LOGI(TAG, "Broadcasting authentication information (%d/5)...", i +
  //   1); espnow_broadcast_auth(); vTaskDelay(pdMS_TO_TICKS(2000)); // Wait 2
...
void vTaskESPNOW_TX(void *pvParameters) {
   ...
   clearMessageQueue();
    ...
    while (1) {
        if (xQueueReceive(espnow_queue, &sensor_data, 0) == pdTRUE) {
            // Prepare formatted sensor data string
            char formatted_str[128];
            int len = snprintf(formatted_str, sizeof(formatted_str),
                               "PCB:%s Count:%d S[%d]",
                               own_pcb_name,
                               message_count++,
                               sensor_data.soil_moisture);

            if (len >= sizeof(formatted_str)) {
                ESP_LOGE(TAG, "Formatted message too long, truncated");
                formatted_str[sizeof(formatted_str) - 1] = '\0';
            }

            // Create and queue a comm_t message
            comm_t message = {
                .address = CONDUCTOR_ADDRESS,
                .command = 0xA3,
                .source = g_nodeAddress,  // global node address
                .retries = 0,
                .seq_num = sequence_number++, // optional: track for ordering
                .data = {0}
            };
            strncpy(message.data, formatted_str, sizeof(message.data) - 1);

            if (xQueueSend(message_queue, &message, 0) != pdPASS) {
                ESP_LOGE(TAG, "Failed to queue message to CONDUCTOR");
            } else {
                ESP_LOGI(TAG, "Queued to CONDUCTOR: %s", message.data);
            }
        }
...
//     // Get own PCB name for inclusion
//     const char *own_pcb_name = espnow_get_peer_name(NULL);
//     espnow_message_t sensor_data;

//     // Get Conductor MAC address once at the start
//     uint8_t conductor_mac[ESP_NOW_ETH_ALEN] = {0};
//     get_mac_for_device(CONDUCTOR_ADDRESS, conductor_mac);

//     if (memcmp(conductor_mac, (uint8_t[ESP_NOW_ETH_ALEN]){0}, ESP_NOW_ETH_ALEN) == 0) {
//         ESP_LOGE(TAG, "Conductor MAC not found, cannot send messages");
//         vTaskDelete(NULL);
//         return;
//     }
...
//                 "PCB:%s Count:%d S[%d]B[%d]T[%d]D[%s]",
//                 own_pcb_name, message_count++,
//                 sensor_data.soil_moisture, sensor_data.battery_level,
//                 sensor_data.temperature, timestamp);
...
//             // Send the message
//             esp_err_t send_result = espnow_send(conductor_mac, (uint8_t *)message, msg_len + 1);
//             if (send_result != ESP_OK) {
//                 ESP_LOGE(TAG, "Failed to send message to conductor: %s", esp_err_to_name(send_result));
//             } else {
//                 ESP_LOGI(TAG, "Sent to CONDUCTOR: %s", message);
//             }
//         }
...
void vTaskESPNOW_RX(void *pvParameters) {
  ...
  while (1) {
    if (message_received) {
      ...
      ESP_LOGD("Sensor", "Processing message from PCB: %s",
               last_sender_pcb_name);

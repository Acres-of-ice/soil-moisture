...
esp_err_t pcf8563_init_desc(i2c_dev_t *dev, i2c_port_t port,
...
esp_err_t pcf8563_get_time(i2c_dev_t *dev, struct tm *time) {
  ...
  esp_err_t res = i2c_master_transmit_receive((dev->dev_handle), &reg, 1, data,
                                              7, I2C_MASTER_TIMEOUT_MS);
  if (res != ESP_OK)
    return res;

  /* convert to unix time structure */
  ESP_LOGD("", "data=%02x %02x %02x %02x %02x %02x %02x", data[0], data[1],
...
uint8_t pcf8563t_read_byte(uint8_t reg) {
  ...
  if (xSemaphoreTake(i2c_mutex, portMAX_DELAY) == pdTRUE) {
    i2c_master_transmit_receive((i2c_dev_rtc.dev_handle), &reg, 1, &data, 1,
...
void pcf8563t_write_byte(uint8_t reg, uint8_t data) {
  ...
  writebuf[1] = data;
  if (xSemaphoreTake(i2c_mutex, portMAX_DELAY) == pdTRUE) {
    i2c_master_transmit((i2c_dev_rtc.dev_handle), writebuf, 2,
                        I2C_MASTER_TIMEOUT_MS);
    xSemaphoreGive(i2c_mutex);
  }
}
...
void read_rtc_time(struct tm *timeinfo) {
  ...
  timeinfo->tm_mon =
      pcf8563t_read_byte(PCF8563T_MONTH) - 1; // Adjust for 0-based months
  timeinfo->tm_year =
      pcf8563t_read_byte(PCF8563T_YEAR) + 2000; // Adjust for year 2000 base
  timeinfo->tm_wday =
      pcf8563t_read_byte(PCF8563T_DAY); // Adjust for day of the week (0-6)
}
...
void set_rtc_time(struct tm *timeinfo) {
  ...
  pcf8563t_write_byte(PCF8563T_MONTH,
                      timeinfo->tm_mon + 1); // Adjust for 1-based months
  pcf8563t_write_byte(PCF8563T_YEAR,
                      timeinfo->tm_year - 2000); // Adjust for year 2000 base
  pcf8563t_write_byte(PCF8563T_DAY,
                      timeinfo->tm_wday); // Adjust for day of the week (0-6)
}
...
char *fetchTime() {
  ...
  if (!initialized) {
    ...
    if (!time_set) {
      struct tm rtcTime = {
          .tm_year = CONFIG_RTC_YEAR,     // Changed: Adding 1900 to match your
                                          // working code

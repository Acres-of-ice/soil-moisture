...
sensor_readings_t readings = {
    .temperature = 99.0f, .humidity = 99.0f, .battery = 99.0f};

sensor_readings_t simulated_readings = {
    .temperature = 15.0f, .humidity = 25.0f, .battery = 0.0f};
...
void sensor_task(void *pvParameters) {
  ...
  ESP_ERROR_CHECK(
      adc_oneshot_config_channel(adc1_handle, EXAMPLE_ADC1_CHAN2, &config));
  ESP_ERROR_CHECK(
      adc_oneshot_config_channel(adc1_handle, EXAMPLE_ADC1_CHAN3, &config));
  ...
  while (1) {
    ...
    ESP_ERROR_CHECK(
        adc_oneshot_read(adc1_handle, EXAMPLE_ADC1_CHAN1, &adc_raw_1[0][0]));
    ESP_ERROR_CHECK(
        adc_oneshot_read(adc1_handle, EXAMPLE_ADC1_CHAN3, &adc_raw_3[0][0]));
...
void dataLoggingTask(void *pvParameters) {
  ...
  while (1) {
    ...
    snprintf(data_entry, sizeof(data_entry), "%.2f,%.2f,%.2f\n",
